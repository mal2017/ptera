...
rule salmon_decoy_mask_exons:
    ...
    """
    input:
        gtf = "results/" + config.get("TRANSCRIPTOME_GTF"),
        genome = "results/" + config.get("GENOME_FASTA")
    output:
        exons = "results/indices/vanilla_salmon_tes_transcripts/decoy/exons.bed",
        genome = "results/indices/vanilla_salmon_tes_transcripts/decoy/genome.fasta",
        masked = "results/indices/vanilla_salmon_tes_transcripts/decoy/txome.masked.genome.fasta"
    singularity:
...
rule salmon_decoy_mashmap_align_txome:
    ...
    input:
        ...
        txpfile = "results/" + config.get("FULL_TRANSCRIPT_FASTA")
    output:
        mm = "results/indices/vanilla_salmon_tes_transcripts/decoy/mashmap.out",
        txlike = "results/indices/vanilla_salmon_tes_transcripts/decoy/genome_found.sorted.bed",
    threads:
        8
    singularity:
...
rule salmon_decoy_finalize:
    ...
    output:
        txpfile = "results/indices/vanilla_salmon_tes_transcripts/decoy/txpfile.fasta",
        txlike_merged = "results/indices/vanilla_salmon_tes_transcripts/decoy/genome_found_merged.bed",
        txlike_fa = "results/indices/vanilla_salmon_tes_transcripts/decoy/genome_found.fasta",
        decoy = "results/indices/vanilla_salmon_tes_transcripts/decoy/decoy.fasta",
        gentrome = "results/indices/vanilla_salmon_tes_transcripts/decoy/gentrome.fasta",
        masked_fai = "results/indices/vanilla_salmon_tes_transcripts/decoy/txome.masked.genome.fasta.fai"
    singularity:
        "docker://quay.io/biocontainers/bedtools:2.30.0--h7d7f7ad_2"
    resources:
        time=10,
...
rule salmon_decoy_get_ids:
    ...
    output:
        ids = "results/indices/vanilla_salmon_tes_transcripts/decoy/decoy.txt"
    priority: 50
    shell:
        """
...
rule salmon_index_transcripts_and_consensus_tes:
    ...
    output:
        directory("results/indices/vanilla_salmon_tes_transcripts/index/")
    params:
        k = config.get("SALMON_K_PARAM"),
        dec = "--decoys results/indices/vanilla_salmon_tes_transcripts/decoy/decoy.txt" if config.get("SALMON_VANILLA_USE_DECOYS") else ""
    singularity:
