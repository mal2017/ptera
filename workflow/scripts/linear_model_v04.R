...
#pairs_fl <- "subworkflows/dgrp_coex/results/linear_models/male_model_01/chunk_0111"
...
#dat_fl <- "subworkflows/dgrp_coex/results/linear_models/male_model_01/expression.tsv.gz"
...
possibly_tidy2 <- possibly(tidy2,NULL)

# custom func for running tls

run_tls <- function(.x,.y) {
  message(.y);
  frm <- as.formula(if_else(length(unique(.x$overlap))==1,alt_formula, formula))
  tls::tls(frm, data = .x, method = "normal")
}

possibly_run_tls <- possibly(run_tls,NULL)
...
res <- df %>% 
  filter(!(feature.y== "R1-2" & feature.x=="FBgn0010770")) %>% # this particular pair kills the computation and I haven't been able to track down what is different about it
  ...
  #mutate(fit=map2(data,feature.x,~{message(.y);tls::tls(as.formula(if_else(length(unique(.x$overlap))==1,alt_formula, formula)), data = ., method = "normal")})) %>%
  mutate(fit=map2(data,feature.x,possibly_run_tls)) %>%
