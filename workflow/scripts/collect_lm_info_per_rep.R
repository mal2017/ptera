...
#x <- 0
#sex <- "male_model_01"
#anov_fl <- sprintf("data/linear_models/%s/%s/lm.anova.tsv.gz",sex,x)
#rainb_fl <- sprintf("data/linear_models/%s/%s/lm.rainbow.tsv.gz",sex,x)
#breusch_fl <- sprintf("data/linear_models/%s/%s/lm.breusch_pagan.tsv.gz",sex,x)
#glance_fl <- sprintf("data/linear_models/%s/%s/lm.glance.tsv.gz",sex,x)
#tidy_correct_fl <- sprintf("data/linear_models/%s/%s/lm.tidy.corrected.tsv.gz",sex,x)
...
rainb <- read_tsv(rainb_fl) %>%
  dplyr::select(feature.x,feature.y,p.value_rainbow=p.value)
