...
rule salmon_decoy_mashmap_align_txome:
    ...
    shell:
        """
        mashmap -r {input.masked} -q {input.txpfile} -t {threads} --pi 80 -s 500 -o {output.mm} &&
...
rule salmon_quant_se_vanilla:
    """
    Baseline gene an TE quantification
    """
    input:
...
# rule vanilla_salmon_cram:
#     input:
#         sam = rules.salmon_quant_se_vanilla.output.sam,
#         ref = rules.make_transcripts_and_consensus_tes_fasta.output
#     output:
#         aln = "results/quantification/vanilla_salmon_tes_transcripts/{sample}/alignments/alignments.sorted.cram",
#         ind = "results/quantification/vanilla_salmon_tes_transcripts/{sample}/alignments/alignments.sorted.cram.crai"
#     threads:
#         8
#     resources:
#         time=60,
...
#     shell:
#         """
#         samtools sort -@ {threads} --reference {input.ref} -O CRAM -o {output.aln} {input.sam} &&
